...
export default function SideBar() {
    ...
    return (
        <div className="w-64 bg-white shadow-md rounded-xl p-4 fixed top-6 left-6 h-[90vh]">
            <nav>
                <ul className="space-y-2">
                    {navItems.map((item, index) => (
                        <li key={index} className="mb-2">
                            <Link href={item.href} className="block px-3 py-2 rounded hover:bg-gray-100 text-gray-800 font-medium">{item.name}</Link>
                        </li>
                    ))}
                </ul>
            </nav>
